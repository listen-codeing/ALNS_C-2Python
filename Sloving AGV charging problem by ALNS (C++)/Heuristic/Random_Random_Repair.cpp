...
#include "Random_Random_Repair.h"
...
Random_Random_Repair::Random_Random_Repair(std::string s)
    : ARepairOperator(s) {
}

Random_Random_Repair::~Random_Random_Repair() {
}

void Random_Random_Repair::repairSolution(ISolution& sol) {
    //cout<<"Random_Random_Repair:"<<endl;
...
void Random_Random_Repair::recalculateCompleteSchedule(AGV_solution& agv_sol){
...
void Random_Random_Repair::processInterval(AGV_solution& agv_sol, int v) {
    auto& agv = agv_sol.agvs[v];
    //??????
    ...
    for (int pos = 0; pos < agv->task_sequence.size()-1; pos++) {
        ...
        if (agv->isCharge[pos]) {
            ...
            if (pos<agv->soc_before_task.size()-2) {
                ...
                if (agv->isCharge[pos+1]) {
                    ...
                    if (min_soc_after_next < agv->soc_at_cs_arrival[pos+1]-1e-3) {
        ...
        if (need_charge && !agv->isCharge[pos]) {
            ...
            std::uniform_int_distribution<int> dist(prev_charge_idx, pos);
            ...
            if (next_charge_pos != -1) {
                ...
                for (int i = pos + 1; i <= next_charge_pos; ++i) {
                    ...
                    if (i == next_charge_pos) {
                        ...
                        // ???????????¨¹????????????????????????
                        if (cur_soc < agv->soc_at_cs_arrival[next_charge_pos]-1e-3) {
                            feasible = false;
                            break;
                        }
                    } else {
...
//                        }
                        ...
                        int n = 20; // ???????
                        double step = (max_charge - min_charge) / n; // ??????
...
int Random_Random_Repair::selectOptimalChargingStation(AGV_solution& agv_sol, int agv_id, int pos, double current_soc) {
...
void Random_Random_Repair::addChargingOperation(AGV_solution& agv_sol,int agv_id, int pos, int station_id) {
    auto& agv = agv_sol.agvs[agv_id];
