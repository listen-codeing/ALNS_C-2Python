...
Greedy_Best_Repair::Greedy_Best_Repair(std::string s)
    : ARepairOperator(s) {
}

Greedy_Best_Repair::~Greedy_Best_Repair() {
}

void Greedy_Best_Repair::repairSolution(ISolution& sol) {
    cout<<"Greedy_Best_Repair"<<endl;
...
void Greedy_Best_Repair::recalculateCompleteSchedule(AGV_solution& agv_sol){
...
void Greedy_Best_Repair::processInterval(AGV_solution& agv_sol, int v) {
    auto& agv = agv_sol.agvs[v];
    //最低电量
    ...
    for (int pos = 0; pos < agv->task_sequence.size()-1; pos++) {
        ...
        if (agv->isCharge[pos]) {
            ...
            //agv->charging_durations[pos] = agv_sol.pm->get_charge_time_min(soc_start, soc_end) + agv_sol.pm->tu;
...
//                    }
//                    else {
//                        agv->charging_durations[pos] = (agv->soc_after_charging[pos]-agv->soc_at_cs_arrival[pos])/agv->pm->eta + agv_sol.pm->tu;
//                    }
                } else {
                    // 可以精确充，计算最优充电量
...
int Greedy_Best_Repair::selectOptimalChargingStation(AGV_solution& agv_sol, int agv_id, int pos, double current_soc) {
...
void Greedy_Best_Repair::addChargingOperation(AGV_solution& agv_sol,int agv_id, int pos, int station_id) {
    auto& agv = agv_sol.agvs[agv_id];
