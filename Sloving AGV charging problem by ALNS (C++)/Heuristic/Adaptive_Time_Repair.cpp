...
#include "Adaptive_Time_Repair.h"
...
#include "../TestUnit.h"
//double flag = false;
Adaptive_Time_Repair::Adaptive_Time_Repair(std::string s)
    : ARepairOperator(s) {
}

Adaptive_Time_Repair::~Adaptive_Time_Repair() {
}
void Adaptive_Time_Repair::repairSolution(ISolution& sol) {
    //cout<<"Adaptive_Time_Repair:"<<endl;

    AGV_solution& agv_sol = dynamic_cast<AGV_solution&>(sol);
    determin_array.clear();
...
void Adaptive_Time_Repair::parallelEventDrivenRepair(AGV_solution& agv_sol) {
    // 初始化事件驱动变量
...
int Adaptive_Time_Repair::findNextTruck(AGV_solution& agv_sol,
...
void Adaptive_Time_Repair::processTaskWithTimeOptimalCharging(
    ...
    //     cout<<"agv->soc_before_task["<<task_idx<<"]:"<<agv->soc_before_task[task_idx]<<endl;
    //     cout<<"here1 error"<<endl;
    //     system("pause");
    // }
    // if (agv->soc_after_task[task_idx] < agv_sol.pm->gamma-1e-3) {
    //     cout<<"agv->soc_after_task["<<task_idx<<"]:"<<agv->soc_after_task[task_idx]<<endl;
    ...
    if (agv->isCharge[task_idx]||task_idx==agv_sol.pm->J_v[agv_id]-1) {
        if (agv->isCharge[task_idx]) {
            ...
            //     cout<<"selected_station error1:"<<selected_station<<endl;
            //     system("pause");
            // }
            ...
            // if (agv->soc_after_task[task_idx] - energy_to_station < agv_sol.pm->gamma-1e-3) {
            //     cout<<"selected_station:"<<selected_station<<endl;
            //     cout<<"here3 error"<<endl;
            //     system("pause");
            // }
        }
        ...
        if (must_charge) {
            ...
            double soc_task = agv->soc_after_task[task_idx];
            bool possible_charge =getPossibleChargeWithSoc(agv_sol, soc_task);
            if (possible_charge&&!isMatch(agv_id, task_idx)) {
                // 如果可能充电，选择时间最优的充电站
                selected_station = selectTimeOptimalStation(agv_sol, agv_id, task_idx,
                                                          agv->soc_after_task[task_idx], task_end_time);
                if (selected_station != -1) {
...
int Adaptive_Time_Repair::selectTimeOptimalStation(AGV_solution& agv_sol,
    ...
    for (int c = 0; c < agv_sol.pm->C; c++) {
        ...
        // cout<<"agv_sol.pm->gamma:"<<agv_sol.pm->gamma<<endl;
        // system("pause");
        if (soc_at_station < agv_sol.pm->gamma-1e-3) {
...
void Adaptive_Time_Repair::calculateTaskTiming(AGV_solution& agv_sol,
                                                      int agv_id, int task_idx,
                                                      double current_time,
                                                      double& arrival_time,
...
void Adaptive_Time_Repair::executeChargingOperation(AGV_solution& agv_sol,
                                                           int agv_id, int task_idx,
    ...
    for (int i = task_idx + 1; i < agv->task_sequence.size()-1; ++i) {
        if (agv->isCharge[i]) {
            next_charge_pos = i;
            break;
        }
    ...
    if (next_charge_pos != -1) {
        ...
        if (feasible) {
            ...
            determin_array.emplace_back(agv_id, task_idx,next_charge_pos);
...
//            }
            ...
            agv->charging_start_times[task_idx] = agv->soc_at_cs_arrival[task_idx];
        } else {
            // 计算最小充电量
            ...
            if (agv->isCharge[task_idx+1]) {
                int station = agv->charging_sessions[task_idx+1].first;
                min_soc_to_next_cs = agv->task_sequence[task_idx+1]->cr[station];
            } else {
                for (int c = 0; c < agv_sol.pm->C; c++) {
            ...
            double min_charge = next_task->cf[station_id] + next_task->cl + min_soc_to_next_cs + agv_sol.pm->gamma - agv->soc_at_cs_arrival[task_idx];
            ...
            if (max_charge-min_charge<1) {
                ...
                agv->soc_charging_durations[task_idx] = random_charge;
                agv->soc_after_charging[task_idx] = agv->soc_at_cs_arrival[task_idx] + random_charge;
...
//                }
    ...
    //表明后续已经没有
        ...
        if (feasible && cur_soc >= agv_sol.pm->gamma-1e-3) {
            ...
            //     agv->isCharge[task_idx] = false;
            //     agv->charging_sessions[task_idx] = {-1, -1};
            //     // 7. 更新卡车状态
            //     agv_current_soc[agv_id] = agv->soc_after_task[task_idx]; // 充电后满电
            //     agv_current_time[agv_id] = agv->task_end_times[task_idx];
            //     return;
            //
            // }
            if (required_charge > max_charge) required_charge = max_charge;
...
//            }
            //后续可以不用遍历了，所有的操作都结束了，
            determin_array.emplace_back(agv_id,task_idx,agv->task_sequence.size()-1);
            //break;
        } else {
            ...
            if (agv->isCharge[task_idx+1]) {
                ...
                min_soc_to_next_cs = next_task->cr[agv->charging_sessions[task_idx+1].first];
            } else {
                for (int c = 0; c < agv_sol.pm->C; c++) {
...
bool Adaptive_Time_Repair::mustChargeAfterTask(AGV_solution& agv_sol,
...
double Adaptive_Time_Repair::calculateTotalChargingTime(AGV_solution& agv_sol,
...
bool Adaptive_Time_Repair::getPossibleChargeWithSoc(AGV_solution& agv_sol,double soc_task) {
    // 根据 soc_task 和 Pi 计算概率，soc_task 越小，概率越大
    double probability = 1.0 - (soc_task / agv_sol.pm->Pi);
    probability = std::clamp(probability, 0.0, 1.0); // 确保概率在 [0, 1] 范围内
...
bool Adaptive_Time_Repair::isMatch(int v, int pos) {
    for (const auto& triple : determin_array) {
        int first, begin_pos, end_pos;
        std::tie(first, begin_pos, end_pos) = triple;

        if ((v == first) && (pos >= begin_pos && pos <= end_pos)) {
            return true;
        }
    }
    return false;
