#include "Station_Worst_Removal.h"
...
using StationUsageInfo = Station_Worst_Removal::StationUsageInfo;

// 构造函数，接受最小和最大移除比例
Station_Worst_Removal::Station_Worst_Removal(double min_perc, double max_perc, std::string s)
    : ADestroyOperator(min_perc, max_perc, s) {
}

Station_Worst_Removal::~Station_Worst_Removal() {
}

void Station_Worst_Removal::destroySolution(ISolution& sol) {
            ...
            if (agv->isCharge[j]) {
                ...
                if (station_id != -1) {
                    if (!is_station_collected[station_id]) {
                        station_usage_list.push_back({station_id, 0.0});
...
void Station_Worst_Removal::removeAllChargingFromStation(AGV_solution& agv_sol, int station_id) {
...
void Station_Worst_Removal::removeChargingDecision(AGV_solution& agv_sol, int agv_id, int task_id) {
