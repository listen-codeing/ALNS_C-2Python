...
#include "../TestUnit.h"
//double flag = false;
Random_Time_Repair::Random_Time_Repair(std::string s)
    : ARepairOperator(s) {
}

Random_Time_Repair::~Random_Time_Repair() {
}
void Random_Time_Repair::repairSolution(ISolution& sol) {
    //cout<<"Random_Time_Repair:"<<endl;
...
void Random_Time_Repair::parallelEventDrivenRepair(AGV_solution& agv_sol) {
    // 初始化事件驱动变量
...
int Random_Time_Repair::findNextTruck(AGV_solution& agv_sol,
...
void Random_Time_Repair::processTaskWithTimeOptimalCharging(
    ...
    if (agv->isCharge[task_idx]||task_idx==agv_sol.pm->J_v[agv_id]-1) {
        ...
        if (must_charge) {
            ...
            bool possible_charge =getPossibleCharge();
...
int Random_Time_Repair::selectTimeOptimalStation(AGV_solution& agv_sol,
...
void Random_Time_Repair::calculateTaskTiming(AGV_solution& agv_sol,
                                                      int agv_id, int task_idx,
                                                      double current_time,
                                                      double& arrival_time,
...
void Random_Time_Repair::executeChargingOperation(AGV_solution& agv_sol,
                                                           int agv_id, int task_idx,
    ...
    auto& agv = agv_sol.agvs[agv_id];
    Task* current_task = agv->task_sequence[task_idx];
    int orinial_station_id = agv->charging_sessions[task_idx].first;
    // 1. 设置充电标志和充电站
...
bool Random_Time_Repair::mustChargeAfterTask(AGV_solution& agv_sol,
...
double Random_Time_Repair::calculateTotalChargingTime(AGV_solution& agv_sol,
...
bool Random_Time_Repair::getPossibleCharge() {
    // 创建随机数生成器
    static std::random_device rd; // 用于获取随机种子
    static std::mt19937 gen(rd()); // Mersenne Twister 随机数引擎
    static std::uniform_real_distribution<> dis(0.0, 1.0); // 0到1的均匀分布

    // 生成0到1之间的随机数
    double probability = dis(gen);

    // 如果随机数小于0.3，返回true，否则返回false
    return probability <0.3; // 30%的概率返回true，70%的概率返回false，如果是0.5的话，充电次数会太多了
}

bool Random_Time_Repair::isMatch(int v, int pos) {
